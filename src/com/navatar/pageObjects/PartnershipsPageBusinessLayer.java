			...
			if (sendKeys(driver, getPartnershipLegalName(environment,mode,60), partnershipLegalName, "Partnership Legal Name",
				...
				if (sendKeys(driver, getFundTextBox(environment,mode,60), fund, "Fund Text Box", action.BOOLEAN)) {
					if (mode.equalsIgnoreCase(Mode.Lightning.toString())) {
						...
						if (click(driver,
								FindElement(driver,
										"//div[contains(@class,'uiAutocomplete')]//a//div[@title='" + fund
												+ "']",
					...
					if (click(driver, getSaveButton(environment,mode,60), "Save Button", action.BOOLEAN)) {
						if (getPartnershipNameInViewMode(environment,mode,60) != null) {
							String partnershipName = getText(driver, getPartnershipNameInViewMode(environment,mode,60),
